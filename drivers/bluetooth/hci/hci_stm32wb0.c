...
static uint32_t __noinit aci_gatt_adv_nwk_buffer[CFG_BLE_GATT_ADV_NWK_BUFFER_SIZE>>2];
...
static uint8_t check_legacy_extended_call(uint16_t opcode, uint8_t *buffer_out)
{
	...
	if((opcode >= BT_HCI_OP_LE_SET_ADV_PARAM) && (opcode <= BT_HCI_OP_LE_CREATE_CONN)){
	...
	else if((opcode >= BT_HCI_OP_LE_SET_EXT_ADV_PARAM) && (opcode <= BT_HCI_OP_LE_READ_PER_ADV_LIST_SIZE)){
	...
	if(!allowed){
		if(opcode == BT_HCI_OP_LE_CREATE_CONN ||
			 opcode == BT_HCI_OP_LE_EXT_CREATE_CONN||
			...
			buffer_out[3] = 0x0C;
			buffer_out[4] = 0x01;
			sys_put_le16(opcode, buffer_out+5);
			return 7;
		}
		else {
			buffer_out[0] = 0x04;
			...
			buffer_out[2] = 0x04;
			buffer_out[3] = 0x01;
			sys_put_le16(opcode, buffer_out+4);
			buffer_out[6] = 0x0C;
			return 7;
		}
	}
...
/* Process Commands */
static uint16_t process_command(uint8_t *buffer, uint16_t buffer_in_length,
				uint8_t *buffer_out, uint16_t buffer_out_max_length)
{
	...
	sys_put_le16(op_code, buffer_out+5);
	return 7;
}
...
{
	IRQ_DIRECT_CONNECT(RADIO_TIMER_TXRX_WKUP_IRQn, BLE_WKUP_PRIO, RADIO_TIMER_TXRX_WKUP_IRQHandler, BLE_WKUP_FLAGS);
	IRQ_DIRECT_CONNECT(RADIO_TXRX_IRQn, BLE_TX_RX_PRIO, RADIO_TXRX_IRQHandler, BLE_TX_RX_FLAGS);
	IRQ_DIRECT_CONNECT(RADIO_TIMER_CPU_WKUP_IRQn, CPU_WKUP_PRIO, RADIO_TIMER_CPU_WKUP_IRQHandler, CPU_WKUP_FLAGS);
	IRQ_DIRECT_CONNECT(RADIO_TXRX_SEQ_IRQn, BLE_RXTX_SEQ_PRIO, RADIO_TXRX_SEQ_IRQHandler, BLE_RXTX_SEQ_FLAGS);
	IRQ_DIRECT_CONNECT(RADIO_TIMER_ERROR_IRQn, BLE_ERROR_PRIO, RADIO_TIMER_ERROR_IRQHandler, BLE_ERROR_FLAGS);
...
static int bt_hci_stm32wb0_open(const struct device *dev, bt_hci_recv_t recv)
{
	...
	HW_RNG_Init(); //TBD_st: do we need to use entropy driver ?
	HW_AES_Init();
	hpka.Instance = PKA;
	HAL_PKA_Init(&hpka);
	HW_PKA_Init();

 ...
 *
 *	When the "sequence" feature is used, the same round is repeated multiple times.
 *
 ...
 *         - ADC LDO is disabled (if applicable)
 *         - ADC I/O Booster is disabled, and clock is turned off (if applicable)
 *         - ADC-SMPS clock synchronization is disabled (if applicable)
 */
...
/* The value of these definitions is chosen to optimize
 * ll_adc_set_calibration_point - do not change them!
 */
 ...
 */
static inline void ll_adc_set_conversion_channel(ADC_TypeDef *ADCx, uint32_t Conversion, uint32_t Channel)
{
	/**
						...
						uint32_t Range, uint32_t Point)
{
	...
	/* 0 if channel_type == 0 (SINGLE_NEG)
	 * 2 if channel_type == 1 (SINGLE_POS)
	 * 4 if channel_type == 2 (SINGLE_DIFF)
	 */
	const uint32_t type_shift = (1 << Type) & ~(Type - 1);

	/* range_shift takes the following values:
	 *  0 (0b0000) if channel_vin_range == 0 (1V2)
	 *  6 (0b0110) if channel_vin_range == 2 (2V4)
	 * 12 (0b1100) if channel_vin_range == 3 (3V6)
	 *
	 * To achieve this, we first calculate tmp:
	 *  0 (0b0000) if channel_vin_range = 0
	 *  1 (0b0001) if channel_vin_range = 2
	 *  2 (0b0010) if channel_vin_range = 3
	 * (functionally, this is equivalent to POPCNT)
	 *
	 * which yields k = ((tmp << 1) | tmp):
	 *  0 (0b0000) if channel_vin_range = 0
	 *  3 (0b0011) if channel_vin_range = 2
	 *  6 (0b0110) if channel_vin_range = 3
	 */
	const uint32_t tmp = ((Range & 2) >> 1) + (Range & 1);
	const uint32_t range_shift = ((tmp << 1) | tmp) << 1;

	//TODO: remove this link
	//Corectness verification available here: https://godbolt.org/z/b5Tvs6EzM
	const uint32_t shift = (range_shift + type_shift);
 ...
 */
static void ll_adc_config_calib_point_from_flash(ADC_TypeDef *ADCx, uint32_t Point,
...
//TODO: rework this during PM implementation
...
/**
 * @brief Schedule as much samplings as possible in a sequence and start the
 * ADC conversion.
 */
static void schedule_and_start_adc_sequence(ADC_TypeDef *adc, struct adc_stm32wb0_data *data)
{
		...
		channel++)
	{
		...
		if (channel == LL_ADC_CHANNEL_TEMPSENSOR) {
			if (calib_pt_ch_type[calib_pt] != ADC_CHANNEL_TYPE_INVALID) {
				 ...
				 *
				 * If this check succeeds, such a channel is scheduled, so we must
				 * delay the temperature sensor to the next sequence. Otherwise, no
				 * such channel has been scheduled and the temperature sensor can be
				 * added to this sequence. It is safe to do it now since the temperature
				 * sensor is the last channel.
				 */
				break;
			} else {
	...
	for (unsigned i = 0; i < NUM_CALIBRATION_POINTS; i++) {
		...
		{
			/* Configure temperature sensor calibration point */
			configure_tempsensor_calib_point(adc, i);
		} else {
			ll_adc_config_calib_point_from_flash(adc, i, type, range);
		}
	}
	...
	if (err < 0) {
		...
		//TODO: is this good enough to handle such errors?
...
int adc_stm32wb0_init(const struct device *dev)
{
	...
	if (config->pinctrl_cfg->state_cnt > 0) {
	...
	/* Keep new data on overrun (should never happen anyways...) */
			       ...
			       enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		//TODO: return adc_stm32_suspend_setup(dev);
	default:
		return -ENOTSUP;
	}
